...
#include "edge_detection_omp.h"

using namespace std;
using namespace cv;

Mat EdgeDetectionOmp::getEdge(cv::Mat frame) {
...
Mat EdgeDetectionOmp::toGrayScale()
{
...
vector<vector<double>> EdgeDetectionOmp::createFilter(int row, int column, double sigmaIn)
{
    ...
    omp_set_num_threads(4);
    #pragma omp parallel for  \
    schedule(dynamic,chunk) shared(filter)
    for (int i = 0; i < row; i++)
    {
        vector<double> col;
...
Mat EdgeDetectionOmp::useFilter(Mat img_in, vector<vector<double>> filterIn)
{
    ...
    Mat filteredImg = Mat(img_in.rows - 2*size, img_in.cols - 2*size, CV_8UC1);
            ...
            #pragma omp parallel for  \
                schedule(dynamic,chunk) \
                collapse(2) shared(filteredImg, size)
...
Mat EdgeDetectionOmp::sobel()
{
    ...
    angles = Mat(gFiltered.rows - 2*size, gFiltered.cols - 2*size, CV_32FC1); //AngleMap
            ...
            #pragma omp parallel for  \
                schedule(dynamic,chunk) \
                collapse(2) shared(size, angles, filteredImg, yFilter,xFilter)
...
Mat EdgeDetectionOmp::nonMaxSupp()
{
    ...
    #pragma omp parallel for  \
    schedule(dynamic,chunk) \
    collapse(2) shared(nonMaxSupped)
...
Mat EdgeDetectionOmp::threshold(Mat imgin,int low, int high)
{
    if(low > 255)
        low = 255;
    if(high > 255)
    ...
    #pragma omp parallel for  \
    schedule(dynamic,chunk) \
    collapse(2) shared(EdgeMat, low, high)
